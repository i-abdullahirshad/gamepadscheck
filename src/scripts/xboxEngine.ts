export class XboxEngine {
    ...
    private initListeners() {
        // Xbox Hardware Warning Modal Dismissal
        const dismissBtn = document.getElementById('btn-stay');
        ...
        window.addEventListener('gamepadconnected', (e) => {
            ...
            // XBOX HARDWARE CHECK
            const isXbox = /xbox|xinput|045e/i.test(pad.id);
            if (!isXbox && modal) {
                const nameSpan = document.getElementById('detected-name');
                if (nameSpan) nameSpan.innerText = pad.id.split('(')[0].trim();
            ...
            if (this.gamepadIndex === null) {
                ...
                if (statusEl) {
                    statusEl.innerText = `🟢 Connected: ${pad.id}`;
    ...
    private calculateError(radiiArray: number[], axisX: number, axisY: number, errorElementId: string) {
        ...
        if (radiiArray.length > 0) {
            ...
            if (errEl) {
                ...
                errEl.style.color = error > 10 ? '#E74C3C' : '#10B981'; // Changed to Emerald Green
            }
        }
    }
    ...
    private updateState() {
        ...
        this.drawCircularity('rs-circ-canvas', pad.axes[2], pad.axes[3], this.rsCircPath, '#10B981'); // Emerald Green Right Stick
        ...
        if (rawContainer) {
            ...
            if (activeButtons.length > 0) {
                rawContainer.innerHTML = activeButtons.map(b => 
                    `<span style="background: #10B981; color: #FFF; padding: 4px 8px; border-radius: 4px; font-weight: 600;">${b}</span>`
                ).join(' ');
            } else {
        ...
        pad.buttons.forEach((btn, index) => {
            ...
            if (svgBtn) {
                svgBtn.style.color = btn.pressed ? '#10B981' : 'var(--border-color)';
            }
        });
